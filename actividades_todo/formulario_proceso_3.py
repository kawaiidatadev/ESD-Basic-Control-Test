...
# Rutas de archivos
plantilla_form3 = r'\\mercury\Mtto_Prod\00_Departamento_Mantenimiento\ESD\Software\Recurses\plantilla_proceso3\form_3.xlsx'
ubicacion_copias = r'\\mercury\Mtto_Prod\00_Departamento_Mantenimiento\ESD\Software\Data\Formularios generados\proces3_form3'
...
def obtener_numero_de_registros():
    ...
    while True:
        try:
            entrada = simpledialog.askinteger("Registros", "Ingresa la cantidad de carritos ESD:             ",
...
def llenar_excel_con_datos(ruta_archivo, numero_de_registros):
    ...
    # Buscar la tabla "Tabla3"
    tabla_encontrada = None
    for table in sheet.tables.values():
        if table.name == "Tabla3":
            tabla_encontrada = table
            break

    if tabla_encontrada is None:
        messagebox.showerror("Error", "No se encontró la tabla 'Tabla3'.")
        return
    ...
    if indice_columna_serie is None:
        messagebox.showerror("Error", "No se encontró la columna 'Numero De Serie' en la tabla 'Tabla3'.")
        return
...
def iniciar_formulario_con_reintentos_f3(max_intentos=3):
    ...
    while intentos < max_intentos:
        try:
            ...
            nuevo_nombre = f"{fecha_hora_actual} Proceso 3 - Form 3.xlsx"
            nueva_ruta = os.path.join(ubicacion_copias, nuevo_nombre)

            shutil.copy(plantilla_form3, nueva_ruta)
