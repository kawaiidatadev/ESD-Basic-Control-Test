...
from actividades_todo.estatus_proceso4 import manejo_de_estatus4
from actividades_todo.db_write_registros_proceso_4 import db_proceso_4_registro


# Zona horaria de Guadalajara, Jalisco, México
tz = pytz.timezone('America/Mexico_City')

def iniciar_p4(ventana_procedimiento_actividad, global_estatus_titulo):
    ...
    if respuesta:  # Si el usuario elige 'Sí'
        ...
        tk.Label(proceso_4, text=f"Procedimiento interno ID: {global_estatus_titulo}", font=("Arial", 14, "bold")).pack(pady=10)

        # Mostrar la fecha y hora actual
        fecha_hora_label = tk.Label(proceso_4, font=("Arial", 12))
        fecha_hora_label.pack()

        # Mostrar el usuario de Windows
        tk.Label(proceso_4, text=f"Usuario: {usuario_windows}", font=("Arial", 12)).pack()
        ...
        btn_salir = tk.Button(proceso_4, text="Salir", command=salir_programa,
        ...
        def actualizar_fecha_hora():
            ...
            proceso_4.after(1000, actualizar_fecha_hora)  # Actualizar cada segundo

        # Frame para los registros con barra de desplazamiento
        frame_registros = tk.Frame(proceso_4)
        ...
        def nuevo_registro():
            top_nuevo = tk.Toplevel(proceso_4)
            ...
            def guardar_registro():
                ...
                # Mostrar el registro en la ventana principal (proceso_2)
        ...
        btn_nuevo_registro = tk.Button(proceso_4, text="Nuevo registro de medición", command=nuevo_registro, font=("Arial", 12, "bold"), bg="green", fg="white", height=2, width=25)
        ...
        def confirmar_registros():
            ...
            if respuesta:
                db_proceso_4_registro(registros)  # Llamar a la función con los registros
                messagebox.showinfo("Éxito", "Todos los registros han sido guardados correctamente.")
                proceso_4.withdraw()
                ventana_procedimiento_actividad.deiconify()

        # Botón para confirmar registros
        btn_confirmar = tk.Button(proceso_4, text="Confirmar registros", command=confirmar_registros, font=("Arial", 12, "bold"), bg="orange", fg="white", height=2, width=25)
